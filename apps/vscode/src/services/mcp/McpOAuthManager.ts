...
// key, in the format @plinycode/core (CLI, JetBrains) reads and writes:
//
...
//  - Writes scope to ONE server's `oauth` key via @plinycode/core's
...
//    @plinycode/core's authorizeMcpServerOAuth, which binds a local loopback
 ...
 * providers. Matches @plinycode/core's DEFAULT_HTTP_MCP_REDIRECT_URL — the actual
 ...
 * @plinycode/core defaults; extras tolerate concurrent flows from other PlinyCode
 ...
 * reads/writes the shared settings file in @plinycode/core's format.
 *
 ...
 * HTTP-callback authorization flow via @plinycode/core.
 */
export class McpOAuthManager {
	 ...
	 *
	 * Delegates to @plinycode/core's authorizeMcpServerOAuth (the exact flow the
