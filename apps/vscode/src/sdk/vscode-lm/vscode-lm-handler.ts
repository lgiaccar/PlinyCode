// PlinyCode SDK `ApiHandler` (from `@plinycode/llms`) backed by the VS Code Language
...
import type { ApiHandler, ApiStreamChunk, HandlerModelInfo, Message, ProviderConfig, ToolDefinition } from "@plinycode/llms"
...
export class VsCodeLmHandler implements ApiHandler {
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		...
		try {
			...
			throw new Error(`PlinyCode <Language Model API>: Failed to select model: ${message}`)
		}
	}
	...
	async *createMessage(systemPrompt: string, messages: Message[], tools?: ToolDefinition[]): AsyncGenerator<ApiStreamChunk> {
		...
		try {
			const requestOptions: vscode.LanguageModelChatRequestOptions = {
				justification: `PlinyCode would like to use '${client.name}' from '${client.vendor}', Click 'Allow' to proceed.`,
			}
			...
			if (error instanceof vscode.CancellationError) {
				throw new Error("PlinyCode <Language Model API>: Request cancelled by user")
			}
			if (error instanceof Error) {
				Logger.error("PlinyCode <Language Model API>: Stream error:", error)
				throw error
			}
			throw new Error(`PlinyCode <Language Model API>: Response stream error: ${String(error)}`)
		} finally {
			this.ensureCleanState()
		}
	}
