# [experimental] @plinycode/llms

`@plinycode/llms` is the model and provider layer for the Cline SDK. It gives
...
- `@plinycode/llms/runtime` for declarative config and runtime registry creation
- `@plinycode/llms/providers` for handler creation and provider settings/types
- `@plinycode/llms/models` for model catalogs and query helpers
- `@plinycode/llms` root exports for the gateway registry and shared llm contracts

## Installation

```bash
npm install @plinycode/llms zod
```

## Quick Start

```ts
import { createHandler } from "@plinycode/llms";
...
```ts
import { createLlmsRuntime, defineLlmsConfig } from "@plinycode/llms/runtime";
```

### Providers

Use `@plinycode/llms/providers` for:
...
`@plinycode/llms` and `@plinycode/shared`.

### Models

Use `@plinycode/llms/models` when you need generated provider/model metadata for
...
```ts
import { transcribeAudio } from "@plinycode/llms";
...
```ts
import { createStreamingAudioTranscriptionSession } from "@plinycode/llms";
...
- `@plinycode/llms`: runtime-focused convenience entrypoint
- `@plinycode/llms/node`: explicit Node/runtime entrypoint
- `@plinycode/llms/browser`: browser-safe bundle
- `@plinycode/llms/runtime`: focused runtime entrypoint
- `@plinycode/llms/models`: model catalog/query entrypoint
- `@plinycode/llms/providers`: provider handler/settings entrypoint
...
bun -F @plinycode/llms run test:live
```
